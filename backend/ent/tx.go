...
func (tx *Tx) init() {
	tx.Eatinghistory = NewEatinghistoryClient(tx.config)
	tx.Foodmenu = NewFoodmenuClient(tx.config)
	tx.Mealplan = NewMealplanClient(tx.config)
	tx.Taste = NewTasteClient(tx.config)
...
// applies a query, for example: Eatinghistory.QueryXXX(), the query will be executed
