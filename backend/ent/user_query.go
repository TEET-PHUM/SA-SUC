...
func (uq *UserQuery) QueryMealplan() *MealplanQuery {
...
func (uq *UserQuery) QueryFoodmenu() *FoodmenuQuery {
	query := &FoodmenuQuery{config: uq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := uq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (uq *UserQuery) QueryEatinghistory() *EatinghistoryQuery {
	query := &EatinghistoryQuery{config: uq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := uq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (uq *UserQuery) WithMealplan(opts ...func(*MealplanQuery)) *UserQuery {
	query := &MealplanQuery{config: uq.config}
	for _, opt := range opts {
		opt(query)
	}
	uq.withMealplan = query
	return uq
}
...
func (uq *UserQuery) WithFoodmenu(opts ...func(*FoodmenuQuery)) *UserQuery {
	query := &FoodmenuQuery{config: uq.config}
	for _, opt := range opts {
		opt(query)
	}
	uq.withFoodmenu = query
	return uq
}
...
func (uq *UserQuery) WithEatinghistory(opts ...func(*EatinghistoryQuery)) *UserQuery {
	query := &EatinghistoryQuery{config: uq.config}
	for _, opt := range opts {
		opt(query)
	}
	uq.withEatinghistory = query
	return uq
}
...
func (uq *UserQuery) sqlAll(ctx context.Context) ([]*User, error) {
	...
	if query := uq.withMealplan; query != nil {
		...
		query.Where(predicate.Mealplan(func(s *sql.Selector) {
			s.Where(sql.InValues(user.MealplanColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			...
			node.Edges.Mealplan = append(node.Edges.Mealplan, n)
		}
	}

	if query := uq.withFoodmenu; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		nodeids := make(map[int]*User)
		for i := range nodes {
			fks = append(fks, nodes[i].ID)
		...
		query.withFKs = true
		query.Where(predicate.Foodmenu(func(s *sql.Selector) {
			s.Where(sql.InValues(user.FoodmenuColumn, fks...))
		}))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		...
		for _, n := range neighbors {
			...
			node.Edges.Foodmenu = append(node.Edges.Foodmenu, n)
		}
	}

	if query := uq.withEatinghistory; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		nodeids := make(map[int]*User)
		for i := range nodes {
			fks = append(fks, nodes[i].ID)
		...
		query.Where(predicate.Eatinghistory(func(s *sql.Selector) {
			s.Where(sql.InValues(user.EatinghistoryColumn, fks...))
		}))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
